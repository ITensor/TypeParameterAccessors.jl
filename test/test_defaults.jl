...
@testset "TypeParameterAccessors defaults" begin
  ...
  @testset "Get defaults" begin
    @test @constinferred(default_type_parameter($Array, 1)) == Float64
    @test @constinferred(default_type_parameter($Array, 2)) == 1
    @test @constinferred(default_type_parameters($Array)) == (Float64, 1)
    @test @constinferred(default_type_parameters($Array, 2, 1)) == (1, Float64)
    @test @constinferred(default_type_parameters($Array, ndims, eltype)) == (1, Float64)
  end
  ...
  @testset "Specify defaults" begin
    ...
    @test @constinferred(specify_default_type_parameters($Array, 1, 2)) == Vector{Float64}
  ...
  @testset "On objects" begin
    ...
    @test @constinferred(default_type_parameter(a, 1)) == Float64
    @test @constinferred(default_type_parameter(a, eltype)) == Float64
    @test @constinferred(default_type_parameter(a, 2)) == 1
    @test @constinferred(default_type_parameter(a, ndims)) == 1
