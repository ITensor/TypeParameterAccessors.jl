...
@testset "Get parameters" begin
  @test @constinferred(type_parameter($(AbstractArray{Float64}), 1)) == Float64
  @test @constinferred(type_parameter($(AbstractArray{Float64}), eltype)) == Float64
  @test @constinferred(type_parameter($(AbstractMatrix{Float64}), ndims)) == 2

  @test @constinferred(type_parameter($(Array{Float64}), 1)) == Float64
  @test @constinferred(type_parameter($(Val{3}))) == 3

  # @test_throws ErrorException type_parameter(Array, 1)
  @test @constinferred(type_parameter($(Array{Float64}), eltype)) == Float64
  @test @constinferred(type_parameter($(Matrix{Float64}), ndims)) == 2
...
@testset "Unspecify parameters" begin
  ...
  @test @constinferred(unspecify_type_parameters($(Vector{Float64}), eltype, 2)) == Array
end

@testset "On objects" begin
  @test @constinferred(type_parameter($(Val{3}()))) == 3
  @test @constinferred(type_parameter($(Val{Float32}()))) == Float32
  a = randn(Float32, (2, 2, 2))
  @test @constinferred(type_parameter(a, 1)) == Float32
  @test @constinferred(type_parameter(a, eltype)) == Float32
  @test @constinferred(type_parameter(a, 2)) == 3
  @test @constinferred(type_parameter(a, ndims)) == 3
  @test @constinferred(type_parameters(a)) == (Float32, 3)
  @test @constinferred(type_parameters(a, 2, eltype)) == (3, Float32)
end
