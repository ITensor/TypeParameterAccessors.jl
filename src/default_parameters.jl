...
function set_default_type_parameter(type::Type, pos)
  return set_type_parameter(type, pos, default_type_parameter(type, pos))
end
function set_default_type_parameters(type::Type)
  return set_type_parameters(type, default_type_parameters(type))
end
function set_default_type_parameters(type::Type, positions::Tuple)
  return set_type_parameters(type, positions, default_type_parameters(type, positions)...)
end

function specify_default_type_parameter(type::Type, pos)
  return specify_type_parameters(type, (pos,), default_type_parameter(type, pos))
end
function specify_default_type_parameters(type::Type)
  return specify_type_parameters(type, default_type_parameters(type))
end
function specify_default_type_parameters(type::Type, positions::Tuple)
  return specify_type_parameters(
    type, positions, default_type_parameters(type, positions)...
  )
end
